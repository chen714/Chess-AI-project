...
*/
    private Stack getWhitePawnSquares(int x, int y, String piece) {
        ...
        if (y == 1) { //if first move
            Square s = new Square(x, y + 1, piece); //one square forward
            moves.push(s);
            Square s1 = new Square(x, y + 2, piece); //two square forward
            moves.push(s1);
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
                    ...
                    if(i == 0){ //2 squares forward
                        if(!(piecePresent(((tmp.getXC() * 75) + 20), ((((tmp.getYC() - 1) * 75) + 20))))){
            ...
            Square s2 = new Square(x, y + 1, piece);
            moves.push(s2);
            Square tmp = (Square) moves.pop();
            Move tmpmove = new Move(startingSquare, tmp);
            if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
            ...
            Square tmp = new Square(x - 1, y + 1, piece);
            Move tmpmove = new Move(startingSquare, tmp);
            if (tmp.getXC() > 0 && piecePresent(((x * 75) + 20 - 75), ((y * 75) + 20 + 75))) {
                if (checkWhiteOponent(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20))) {     //left down
                    if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
            ...
            Square tmp1 = new Square(x + 1, y + 1, piece);
            Move tmpmove1 = new Move(startingSquare, tmp1);
            if (tmp1.getXC() < 7 && piecePresent(((x * 75) + 20  + 75), ((y * 75) + 20 + 75))) {
                if (checkWhiteOponent(((tmp1.getXC() * 75) + 20), ((tmp1.getYC() * 75) + 20))) { //down right
                    if ((tmp1.getXC() < 0) || (tmp1.getXC() > 7) || (tmp1.getYC() < 0) || (tmp1.getYC() > 7)) {

                    } else {
                        attackingMove.push(tmpmove1);
                    }
                }
            }
    ...
    private Stack getWhitePawnAttackingSquaresForKing(int x, int y, String piece, Square kingSquare){
        Square startingSquare = new Square(x, y, piece);
        int kingX = kingSquare.getXC();
        int kingY = kingSquare.getYC();
        Stack attackingMove = new Stack();


        Square tmp = new Square(x - 1, y + 1, piece);
        Move tmpmove = new Move(startingSquare, tmp);
        if ((tmp.getXC() > 0)&&(kingX == tmp.getXC())&&(kingY == tmp.getYC())) {
                //left down
                if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
        ...
        Square tmp1 = new Square(x + 1, y + 1, piece);
        Move tmpmove1 = new Move(startingSquare, tmp1);
        if ((tmp.getXC() < 7)&&(kingX == tmp1.getXC())&&(kingY == tmp1.getYC())) {
             //down right
                if ((tmp1.getXC() < 0) || (tmp1.getXC() > 7) || (tmp1.getYC() < 0) || (tmp1.getYC() > 7)) {
    ...
    private Stack getBlackPawnSquares(int x, int y, String piece) {
        ...
        if (y == 6) { //if first move
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
                    ...
                    if(i == 0){ //2 squares forward
                        if(!(piecePresent(((tmp.getXC() * 75) + 20 + 75), (((tmp.getYC() * 75) + 20 + 75))))){
                            attackingMove.push(tmpmove);
                        }
                    }else {
                        attackingMove.push(tmpmove);
        ...
        Move tmpmove = new Move(startingSquare, tmp);
        if (tmp.getXC() > 0 && piecePresent(((x * 75) + 20 - 75), ((y * 75) + 20 - 75))) {
            if (checkWhiteOponent(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20))) {     //left up
                if ((tmp.getXC() < 0) || (tmp.getXC() > 7) || (tmp.getYC() < 0) || (tmp.getYC() > 7)) {
...
*/
    private Stack getKingSquares(int x, int y, String piece) {
        Square startingSquare = new Square(x, y, piece);
        Stack moves = new Stack();
        Stack notInCheckMoves = new Stack();
...
*/
    ...
    */
        ...
        while(!moves.empty()){
            Move tempMove = (Move)moves.pop();
            int whiteX = tempMove.landing.getXC();
            int whiteY = tempMove.landing.getYC();
            Boolean inCheck = false;
            Stack blackPossibleMoves = getBlackPossibleMoves();

            while(!blackPossibleMoves.empty()){
                Move tempBlackMove = (Move) blackPossibleMoves.pop();
                int blackX = tempBlackMove.landing.getXC();
                int blackY = tempBlackMove.landing.getYC();
                if((whiteX == blackX) && (whiteY == blackY)){
                    inCheck = true;
                }
            }
            if(!inCheck){
                notInCheckMoves.push(tempMove);
            }
        }
    ...
    private Boolean checkSurroundingSquaresForBlack(Square s) {
        Boolean possible = false;
        int x = s.getXC() * 75;
        int y = s.getYC() * 75;
    ...
    private Stack getKingSquaresForBlack(int x, int y, String piece) {
        Square startingSquare = new Square(x, y, piece);
        Stack moves = new Stack();
        Move validM, validM2, validM3, validM4;
        int tmpx1 = x + 1;
        ...
        if (!((tmpx1 > 7))) {
            ...
            Square tmp1 = new Square(tmpx1, tmpy1, piece);
            Square tmp2 = new Square(tmpx1, tmpy2, piece);
            if (checkSurroundingSquaresForBlack(tmp)) {
                validM = new Move(startingSquare, tmp);
                if (!piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {
                    moves.push(validM);
                } else {
        ...
        return moves;
    } // end of the method getKingSquaresForBlack()
...
*/
  ...
  */
    ...
    private Stack getQueenMovesForBlack(int x, int y, String piece) {
        Stack completeMoves = new Stack();
        Stack tmpMoves = new Stack();
        Move tmp;
        tmpMoves = getRookMovesForBlack(x, y, piece);
        while (!tmpMoves.empty()) {
            tmp = (Move) tmpMoves.pop();
            completeMoves.push(tmp);
        }
        tmpMoves = getBishopMovesForBlack(x, y, piece);
        while (!tmpMoves.empty()) {
            tmp = (Move) tmpMoves.pop();
            completeMoves.push(tmp);
        }
...
*/
  ...
  */
    ...
    private Stack getRookMovesForBlack(int x, int y, String piece) {
        Square startingSquare = new Square(x, y, piece);
        Stack moves = new Stack();
        Move validM, validM2, validM3, validM4;
...
*/
  ...
  */
    ...
    private Stack getBishopMovesForBlack(int x, int y, String piece) {
        Square startingSquare = new Square(x, y, piece);
        Stack moves = new Stack();
        Move validM, validM2, validM3, validM4;
        for (int i = 1; i < 8; i++) {
...
*/
    ...
    private Stack getKnightMovesForBlack(int x, int y, String piece) {
        Square startingSquare = new Square(x, y, piece);
        Stack moves = new Stack();
        Stack attackingMove = new Stack();
        Square s = new Square(x + 1, y + 2, piece);
...
*/
    private Stack findWhitePieces() {
        ...
        for (int i = 0; i < 700; i += 75) {
            for (int j = 0; j < 700; j += 75) {
                y = i / 75;
                x = j / 75;
                Component tmp = chessBoard.findComponentAt(j, i);
                if(tmp instanceof JLabel) {
                    chessPeice12 = (JLabel) tmp;
                    icon = chessPeice12.getIcon().toString();
    ...
    public Boolean isKingInCheck(Square kingSquare, Stack white){
        Boolean inCheck = false;
        Stack completeMoves = new Stack();
        int landingX = kingSquare.getXC();
        int landingY = kingSquare.getYC();
        Move tmp;
      ...
      */
            if (tmpString.contains("Knight")) {
                ...
                tmpMoves = getWhitePawnAttackingSquaresForKing(s.getXC(), s.getYC(), s.getName(), kingSquare);
        ...
        Stack  copyBlack = (Stack) completeMoves.clone();
        getLandingSquares(completeMoves);

        panels = (JPanel) chessBoard.getComponent(panelID);
        Border redBorder = BorderFactory.createLineBorder(Color.RED, 3);
        panels.setBorder(redBorder);
        while (!copyBlack.empty()) {
            Move tempWhiteMove = (Move) copyBlack.pop();
            int whiteX = tempWhiteMove.landing.getXC();
            int whiteY = tempWhiteMove.landing.getYC();
            if ((landingX == whiteX) && (landingY == whiteY)) {
                inCheck = true;
            }
        }
    ...
    public Stack getWhitePossibleMoves(){
    ...
    public Stack getBlackPossibleMoves(){
        Stack blackPieces = findBlackPieces();
        Stack blackPossibleMovements = new Stack();
        Move tmp;


        while(!blackPieces.empty()){
            Stack tmpBlackMove = new Stack();
            Square tmpBlackPeice = (Square)blackPieces.pop();
            String peiceName = tmpBlackPeice.pieceName;

            if (peiceName.contains("Knight")) {
                tmpBlackMove = getKnightMovesForBlack(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            } else if (peiceName.contains("Bishop")) {
                tmpBlackMove = getBishopMovesForBlack(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            } else if (peiceName.contains("Pawn")) {
                tmpBlackMove = getBlackPawnSquares(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            } else if (peiceName.contains("Rook")) {
                tmpBlackMove = getRookMovesForBlack(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            } else if (peiceName.contains("Queen")) {
                tmpBlackMove = getQueenMovesForBlack(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            } else if (peiceName.contains("King")) {
                tmpBlackMove = getKingSquaresForBlack(tmpBlackPeice.getXC(), tmpBlackPeice.getYC(), tmpBlackPeice.getName());
            }

            while (!tmpBlackMove.empty()) {
                tmp = (Move) tmpBlackMove.pop();
                blackPossibleMovements.push(tmp);
            }

        }
        return blackPossibleMovements;
    }
...
*/
    ...
    */
            ...
            if(n == 0){
                ...
                System.out.println("Currently using random agent. ");
            }else if(n == 1){
                Stack blackPieces = findBlackPieces();
                selectedMove = agent.nextBestMove(testing, blackPieces);
            }else if(n == 2){
                Stack blackPieces = findBlackPieces();
                Stack blackPossibleMovements = getBlackPossibleMoves();
                Stack whitePieces = findWhitePieces();

                selectedMove = agent.twoLevelsDeep(testing, blackPossibleMovements, findBlackPieces(), whitePieces);
            }
            ...
            parent.remove(c);
            panelID = (startingPoint.getYC() * 8) + startingPoint.getXC();
            panels = (JPanel) chessBoard.getComponent(panelID);
            panels.setBorder(redBorder);
            parent.validate();
            ...
            if (l instanceof JLabel) {
                ...
                if(selectedMove.landing.pieceName.contains("WhitePawn")&&(selectedMove.getLanding().getYC() == 7)){
                    Component c1 = (JLabel) chessBoard.findComponentAt(landingX1, landingY1);
                    JLabel pieces1;
                    Container parent1 = c1.getParent();
                    int location1 = landingPanelID;
                    if (c1 instanceof JLabel) {
                        parent1.remove(c1);
                        pieces1 = new JLabel(new ImageIcon("WhiteQueen.png"));
                        parent1 = (JPanel) chessBoard.getComponent(location1);
                        parent1.add(pieces1);
                    } else {
                        parent1 = (Container) c1;
                        pieces1 = new JLabel(new ImageIcon("WhiteQueen.png"));
                        parent1 = (JPanel) chessBoard.getComponent(location1);
                        parent1.add(pieces1);
                    }
        ...
        if (possible) {
          ...
          */
                ...
                if ((xMovement == 0) && (yMovement == 0)) {
                    ...
                    if (((e.getX() < 0)||(e.getX()>600)|| (e.getY() < 0)||(e.getY()>600) ||(landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
                        validMove = false;
                    } else {
                        if ((xMovement > 1) || (yMovement > 1)) {
                            ...
                            layeredPane.validate();
                            layeredPane.repaint();
                            Stack whitePeices = findWhitePieces();
                            Square kingSquare = new Square(landingX, landingY, pieceName);
                            if (isKingInCheck(kingSquare, whitePeices)) {
